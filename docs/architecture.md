...
На стенде `mongo-sharding-repl` проверено: 200 запросов с `secondaryPreferred` через
mongos разошлись по `shard1-2` и `shard1-3` примерно поровну, primary их не получил.
...
| `products` | товар в админке после редактирования | primary | 0 | менеджер должен увидеть свои изменения сразу |
...
| `carts` | счётчик товаров в шапке сайта | primary или кеш redis | 0 | расхождение со страницей корзины видно сразу; дешевле кешировать по `owner_key`, чем читать с secondary |
...
меньше 90 секунд, на стенде драйвер отвечает `maxStalenessSeconds value cannot be less
than 90`. Он выставляется как страховка от совсем отставшей ноды, а рабочие пороги
контролирует мониторинг.
...
# переключение классов B и C на primary по тревоге отставания
def catalog_collection():
    if settings.replication_lag_alert:
        return db.products
    return products_ro
```
